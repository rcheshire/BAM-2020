...
tplfile.name=paste(datin$setup$run.name,"tpl",sep=".")

#functions to write to text files, best use for less than 6 - no wrapping
datf=function(x){write(x,file=datfile.name)}
tplf=function(x){write(x,file=tplfile.name)}

# write out *.dat file
datf("# Data Input File")
tplf(x="# Tpl File")

datf(x=paste("#",datin$setup$sedar,"South Atlantic", datin$setup$species, sep=" "))
tplf(x=paste("#",datin$setup$sedar,"South Atlantic", datin$setup$species, sep=" "))
...
datf(x="#Starting and ending year of model")
tplf(x="#Starting and ending year of model")

datf(x=datin$setup$styr)
tplf("styr")
