...
open_r_info_list("parms", false);
	...
	wrt_r_item("F.prop.cO", F_cO_prop);	
...
open_r_df("parm.cons",1,8,2);
	...
	wrt_r_df_col("log_dm_cO_lc",log_dm_cO_lc_out);
	wrt_r_df_col("log_dm_HB_lc",log_dm_HB_lc_out);
	wrt_r_df_col("log_dm_GR_lc",log_dm_GR_lc_out);
    ...
    wrt_r_df_col("selpar_A50_cO2",selpar_A50_cO2_out);
    wrt_r_df_col("selpar_A50_cO3",selpar_A50_cO3_out);	
    wrt_r_df_col("selpar_slope1_cO2",selpar_slope_cO2_out);
	wrt_r_df_col("selpar_A502_cO2",selpar_A502_cO2_out);
	wrt_r_df_col("selpar_slope2_cO2",selpar_slope2_cO2_out);
	...
	wrt_r_df_col("log_avg_F_cO",log_avg_F_cO_out);
...
open_r_df("parm.tvec", styr, endyr, 2);
	...
	wrt_r_df_col("log.F.dev.cO", log_F_dev_cO_out);
...
open_r_df("parm.tvec.cons",1,3,2);
	...
	wrt_r_df_col("log.F.dev.cO",set_log_F_dev_cO);
...
open_r_vector("sdnr");
    ...
    wrt_r_item("sdnr.lc.cO", sdnr_lc_cO);	
	wrt_r_item("sdnr.lc.HB", sdnr_lc_HB);
	wrt_r_item("sdnr.lc.GR", sdnr_lc_GR);	
...
open_r_vector("like");
    ...
    wrt_r_item("lk.lenc.cO", f_cO_lenc);	
	wrt_r_item("lk.lenc.HB", f_HB_lenc);
	wrt_r_item("lk.lenc.GR", f_GR_lenc);	
    ...
    wrt_r_item("w.lc.cO", w_lc_cO);	
	wrt_r_item("w.lc.HB", w_lc_HB);
	wrt_r_item("w.lc.GR", w_lc_GR);
...
open_r_list("size.age.fishery");
    ...
    open_r_matrix("len.cO.mm");
    wrt_r_matrix(len_cO_mm, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);
    close_r_matrix();
	...
	open_r_matrix("wholewgt.cO.lb");
    wrt_r_matrix(wholewgt_cO_klb*1000.0, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);
    close_r_matrix();
...
open_r_list("sel.age");
	...
	open_r_matrix("sel.m.cO");
    wrt_r_matrix(sel_cO, 2, 2);
    wrt_r_namevector(styr,endyr);
    wrt_r_namevector(agebins);
    close_r_matrix();
...
open_r_list("comp.mats");
    ...
    open_r_matrix("lcomp.cO.ob");
    wrt_r_matrix(obs_cO_lenc, 2, 2);
    wrt_r_namevector(yrs_cO_lenc);
    wrt_r_namevector(lenbins);
    close_r_matrix();

    open_r_matrix("lcomp.cO.pr");
    wrt_r_matrix(pred_cO_lenc, 2, 2);
    wrt_r_namevector(yrs_cO_lenc);
    wrt_r_namevector(lenbins);
    close_r_matrix();
	...
	open_r_matrix("lcomp.GR.ob");
    wrt_r_matrix(obs_GR_lenc, 2, 2);
    wrt_r_namevector(yrs_GR_lenc);
    wrt_r_namevector(lenbins);
    close_r_matrix();

    open_r_matrix("lcomp.GR.pr");
    wrt_r_matrix(pred_GR_lenc, 2, 2);
    wrt_r_namevector(yrs_GR_lenc);
    wrt_r_namevector(lenbins);
    close_r_matrix();
...
open_r_df("t.series", styr, (endyr+1), 2);
	...
	wrt_r_df_col("L.cO.ob", obs_cO_L);
    wrt_r_df_col("L.cO.pr", pred_cO_L_klb);
    wrt_r_df_col("cv.L.cO", cO_L_cv);
	...
	wrt_r_df_col("lcomp.cO.n", nsamp_cO_lenc_allyr);
	wrt_r_df_col("lcomp.HB.n", nsamp_HB_lenc_allyr);
	wrt_r_df_col("lcomp.GR.n", nsamp_GR_lenc_allyr);
	...
	wrt_r_df_col("lcomp.cO.nfish", nfish_cO_lenc_allyr);
	wrt_r_df_col("lcomp.HB.nfish", nfish_HB_lenc_allyr);
	wrt_r_df_col("lcomp.GR.nfish", nfish_GR_lenc_allyr);
	...
	wrt_r_df_col("lcomp.cO.neff", (1+nsamp_cO_lenc_allyr*exp(log_dm_cO_lc_out(8)))/(1+exp(log_dm_cO_lc_out(8))) );
	wrt_r_df_col("lcomp.HB.neff", (1+nsamp_HB_lenc_allyr*exp(log_dm_HB_lc_out(8)))/(1+exp(log_dm_HB_lc_out(8))) );
	wrt_r_df_col("lcomp.GR.neff", (1+nsamp_GR_lenc_allyr*exp(log_dm_GR_lc_out(8)))/(1+exp(log_dm_GR_lc_out(8))) );
...
open_r_df("LD.pr.tseries", styr, endyr, 2);
    ...
    wrt_r_df_col("L.cO.klb", pred_cO_L_klb);
    wrt_r_df_col("L.cO.knum", pred_cO_L_knum);	
